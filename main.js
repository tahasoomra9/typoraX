...
timeSelect.forEach(btn => btn.addEventListener('click', () => {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (key == 'Backspace') {
      ...
      if (currentIndex > 0) {
        ...
        if (spans[currentIndex].classList.contains('correct')) { // if spans contian correct when backspaced

          
          correctKeys--; //minus the corrected keys too
        }

        if (spans[currentIndex].classList.contains('incorrect')) { // if current index or span is incorrect and backspace is clicked
